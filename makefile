...
build:all
	...
	rm onix/boot/grub/ -r -f
	mkdir onix/boot/grub/
	echo set default=0 >> onix/boot/grub/grub.cfg
	echo set timeout=0 >> onix/boot/grub/grub.cfg
	echo menuentry "onix" { >> onix/boot/grub/grub.cfg
	echo         set root='(hd96)' >> onix/boot/grub/grub.cfg
	echo         multiboot /boot/kernel.bin >> onix/boot/grub/grub.cfg
	echo } >> onix/boot/grub/grub.cfg

	grub-mkrescue -o onix.iso onix/
